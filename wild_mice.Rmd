...
alpha <- read.table(file="16S.analysis/wild_mice.16S.trim.contigs.unique.good.good.filter.unique.precluster.pick.pick.an.unique_list.0.03.subsample.groups.ave-std.summary", header=T)
alpha <- alpha[alpha$method=="ave",]
rownames(alpha) <- gsub("_16S", "", alpha$group)
o <- order(rownames(alpha))
alpha <- alpha[o,]
alpha <- alpha[,-c(1,2,3)]

stopifnot(sum(rownames(md)!=rownames(alpha)) == 0)	#make sure everything is in the same order

sobs.anova <- summary(aov(rank(alpha$sobs) ~ md$SP*md$Sex + Error(md$ET)))
summary(aov(rank(alpha$sobs) ~ md$SP*md$Sex + Error(md$ET)))

shannon.anova <- summary(aov(rank(alpha$shannon) ~ md$SP*md$Sex + Error(md$ET)))
summary(aov(rank(alpha$shannon) ~ md$SP*md$Sex + Error(md$ET)))
...
```{r, warning=FALSE}
subsampled <-read.table(file="16S.analysis/wild_mice.16S.trim.contigs.unique.good.good.filter.unique.precluster.pick.pick.an.unique_list.0.03.subsample.shared", header=T, row.names=2)
subsampled <- subsampled[,-c(1,2)]
rownames(subsampled) <- gsub("_16S", "", rownames(subsampled))
o <- order(rownames(subsampled))
subsampled <- subsampled[o,]
stopifnot(sum(rownames(md)!=rownames(subsampled)) == 0)	#make sure everything is in the same order

nseqs <- apply(subsampled, 1, sum)[1]
relabund <- subsampled/nseqs
...
figure2 <- function(){
	...
	# panel A
	pl.shannon <- aggregate(alpha$shannon[md$SP=="PL"], by=list(md$ET[md$SP=="PL"]), median)$x
	pmg.shannon <- aggregate(alpha$shannon[md$SP=="PMG"], by=list(md$ET[md$SP=="PMG"]), median)$x
	...
	pl.sobs <- aggregate(alpha$sobs[md$SP=="PL"], by=list(md$ET[md$SP=="PL"]), median)$x
	pmg.sobs <- aggregate(alpha$sobs[md$SP=="PMG"], by=list(md$ET[md$SP=="PMG"]), median)$x
	...
	# panel C
	first <- aggregate(subsampled[,most.imp.otus[1]]/nseqs, by=list(md$SP, md$ET), median)
	first.pl <- first[first$Group.1=="PL", "x"]
	first.pmg <- first[first$Group.1=="PMG", "x"]
	first.med <- median(first$x)
	
	second <- aggregate(subsampled[,most.imp.otus[2]]/nseqs, by=list(md$SP, md$ET), median)
	second.pl <- second[second$Group.1=="PL", "x"]
	second.pmg <- second[second$Group.1=="PMG", "x"]
	second.med <- median(second$x)
	
	third <- aggregate(subsampled[,most.imp.otus[3]]/nseqs, by=list(md$SP, md$ET), median)
...
```{r}
total.n.otus <- ncol(subsampled)
distribution <- quantile(alpha$sobs, c(0.025, 0.50, 0.975))
present90 <- apply(subsampled>0, 2, sum) >= 0.90*nrow(subsampled)
n.present90 <- sum(apply(subsampled>0, 2, sum) >= 0.90*nrow(subsampled))
otu.present90 <- colnames(subsampled)[present90]
ntaxa.present90 <- levels(factor(taxa[otu.present90]))
percent.present90 <- 100 * sum(subsampled[,present90])/sum(subsampled)

n.presentAll <- sum(apply(subsampled>0, 2, sum) == nrow(subsampled))
#0


```

One striking feature of the communities observed between the mice was the large amount of variation in the composition of their gut microbiota. We observed a total of `r total.n.otus` OTUs among all of the mice; however, the median number of OTUs observed in each mouse was `r distribution["50%"]` (95% confidence interval=`r distribution["2.5%"]`-`r distribution["97.5%"]`). Across all of the OTUs, no (`r n.presentAll`) OTUs were present in every sample, but `r n.present90` were observed in at least 90% of the mice (Figure 3A). These OTUs affiliated with `r ntaxa.present90` different genera and represented `r format(percent.present90, digits=3)`% of all of the 16S rRNA gene sequences sampled from the mice (Figure 3A). These OTUs did not overlap with the two OTUs that were identified as being significantly different between the two species of mice. We next considered the taxonomic diversity of bacterial genera. Among the 475 genera that were observed, 28 were observed in at least 90% of the mice and 12 were observed in every mouse sampled (Figure 3B). Those genera found in at least 90% of the mice represented 87% of all sequences sampled and those found in every animal represented 75% of all sequences sampled. Within each animal we identified the dominant genus and observed that Lactobacillus was the dominant genus in 39% of the mice followed by Lachnospiraceae (27%) and Porphyromonadaceae (16%). These results demonstrate that among the Peromyscus spp. animals that we sampled, there was a conserved taxonomic core.
